...
atom() ->
    ?assertError(function_clause, mergesort:mergesort(abc)).

integer() ->
    ?assertError(function_clause, mergesort:mergesort(123)).

float() ->
    ?assertError(function_clause, mergesort:mergesort(1.1)).

empty_list() ->
    Input = [],
    Expected = [],
    Actual = mergesort:mergesort(Input),
